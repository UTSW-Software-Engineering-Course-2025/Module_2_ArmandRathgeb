...
class MyImgClass():
    ...
    def __sub__(self, other):
        ...
        :rtype: MyImgClass
        """
        return MyImgClass(np.subtract(self.arrImg, other.arrImg))
    ...
    def fMSE(self, other):
        ...
        :rtype: float
        """
        img = self.fPixelwiseSqDif(other)
        return np.sum(img.arrImg) / img.arrImg.size
    ...
    def fComputeMeanAcrossImages(lMyImgClass):
        ...
        :rtype: MyImgClass
        """
        imgs = np.array([img.arrImg for img in lMyImgClass])
        return MyImgClass(np.mean(imgs, axis=0))
    ...
    def fComputeStdAcrossImages(lMyImgClass):
        ...
        """
        imgs = np.array([img.arrImg for img in lMyImgClass])
        return MyImgClass(np.std(imgs, axis=0))
    ...
    def fMeanMSE(lImg1, lImg2):
        ...
        :rtype: float
        """
        mses = np.array([im1.fMSE(im2) for im1,im2 in zip(lImg1, lImg2)])
        return np.mean(mses)
